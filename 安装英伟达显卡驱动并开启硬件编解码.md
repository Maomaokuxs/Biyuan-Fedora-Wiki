...
⚠️注意：这里安装时需要根据显卡型号选择正确的驱动版本号（一般来说十年内的卡都能用最新驱动），上面的命令安装的是最新驱动，如果需要安装旧版驱动，请搜索对应的包名 比如 xorg-x11-drv-nvidia-470xx akmod-nvidia-470xx xorg-x11-drv-nvidia-470xx-cuda ，请注意旧版需要安装的是三个包。搜索命令可用例如 dnf search akmod-nvidia。
...
⚠️注意：如果不使用安全启动这一部分不用看，即使不开启安全启动可以使用驱动，要关闭需要进入 bios 设置。
